...
def test_simplex_to_chain_complex():
    ...
    for dimension in range(1, max_dim + 1):
        # Check that `∂∂ == 0`
        As = zmsh.polytopal.standard_simplex(dimension)
...
def test_matrix_transformations():
    ...
    p, s = zmsh.polytopal.find_permutation_and_sign(A, B)
...
def test_simplicial_chain_complex_morphism():
    As = zmsh.polytopal.standard_simplex(2)
    Bs = [np.copy(A) for A in As]
    As[1][:, 1] *= -1
    As[2][1, :] *= -1
    ps, ss = zmsh.polytopal.find_isomorphism(As, Bs)
    assert np.array_equal(ss[2], [+1, -1, +1])
...
def test_triangle_orientation_exhaustive():
    ...
    for p, s in zip(permutations(range(3)), product([+1, -1], repeat=3)):
        ...
        B_1 = A_1[:, p] @ np.diag(s)
        B_2 = np.diag(s) @ A_2[p, :]
        assert zmsh.polytopal.orientation([B_0, B_1, B_2]) == +1
...
def test_tetrahedron_orientation_random():
    ...
    A_0, A_1, A_2, A_3 = zmsh.polytopal.standard_simplex(3)
    ...
    for trial in range(num_trials):
        ...
        assert zmsh.polytopal.orientation([B_0, B_1, B_2, B_3]) == +1
        assert zmsh.polytopal.orientation([B_0, B_1, B_2, -B_3]) == -1


def test_topology_orientation():
    Ds = zmsh.polytopal.standard_simplex(2)
    assert zmsh.polytopal.orientation(Ds) == +1
